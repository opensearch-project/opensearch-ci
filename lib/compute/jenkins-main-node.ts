...
export class JenkinsMainNode {
  ...
  public static configElements(stackName: string, stackRegion: string, httpConfigProps: HttpConfigProps,
    ...
    return [
      ...
      // eslint-disable-next-line max-len
      InitCommand.shellCommand('sudo wget -nv https://cache.agilebits.com/dist/1P/op2/pkg/v2.24.0/op_linux_amd64_v2.24.0.zip -O /tmp/op.zip && sudo unzip -j /tmp/op.zip op -d /usr/bin && sudo rm -v /tmp/op.zip'),
      // eslint-disable-next-line max-len
